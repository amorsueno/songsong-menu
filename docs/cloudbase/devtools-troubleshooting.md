...
通常是 `getRecipes`、`getMyRecipes`、`getRecipeDetail` 未部署，或集合权限不匹配。
...
通常是图片没有成功上传到云存储，或 `createRecipe`、`updateRecipe`、`deleteRecipe` 云函数未部署。
...
当前 MVP 允许 AI 缺省退化，所以最常见原因是环境变量没有配置。
...
通常是 `login` 云函数未正常写入用户档案，或本地登录态没有成功保存。
